...
router.route(`/${AUTH_PATH.REGISTER}`).post([validateBody(schema.register)],controller.handleRegister);
router.route(`/${AUTH_PATH.LOGIN}`).post([validateBody(schema.login)],controller.handleLogin);
router.route(`/${AUTH_PATH.VERIFY}`).post([],controller.handleVerify);
router.route(`/${AUTH_PATH.GETOTP}`).get([],controller.handleGetOTP);
router.route(`/${AUTH_PATH.GET_AUTH}`).get([verifyToken],controller.handleGetAuth);
